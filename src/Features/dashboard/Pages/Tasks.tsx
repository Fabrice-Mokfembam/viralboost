  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col items-center py-8 px-2">
      ...
      <div className="w-full max-w-md flex bg-gray-800 p-1 rounded-lg mb-4">
        <button className="flex-1 py-2 text-center text-sm font-semibold bg-cyan-500 text-white rounded-l-lg">
          Undone
        </button>
      ...
      <div className="w-full max-w-md space-y-4">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="flex items-center justify-between bg-gray-800 rounded-lg p-4 shadow border border-gray-700"
          >
            ...
            <button onClick={()=>navigate(`/v/task/${task.id}`)} className="ml-4 bg-cyan-500 hover:bg-cyan-600 text-white font-semibold py-2 px-4 rounded-md shadow">
              Do Task
            </button>
          </div>
        ))}
